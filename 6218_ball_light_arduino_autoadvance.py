...
print('Last updated 18 May 2021 \n \n \n')
...
#prepare analog task to read nine channels. For the 6218 DAQ we want differential reads and I need to figure out how to configure that!
t = nidaq.Task()
t.CreateAIVoltageChan("Dev1/ai0:7, Dev1/ai16", None, nidaq.DAQmx_Val_Diff, -5, 5, nidaq.DAQmx_Val_Volts, None)

#channel order: EMG1, EMG2, EMG3, EMG4, EMG5, Switch, Trigger, Encoder 1, Encoder 2 (EMG% is the trigger module)
...
def getanalog():
    ...
    while read_indicator > 0: #time.clock()-start < 5:
        ...
        s.put(data[6]) #put the second entry, which is the trigger, into the queue that's read to figure out when to terminate trial 
...
for i in range(int(stindex)-1,numberoftrials):    
    ...
    if i == numberoftrials-1:
        ...
        print('Trial ' + str(i+1) + ' Completed and data logged as: ' + revisedname)
        time.sleep(.5) #we go to the top of the loop immediately after ENTER
